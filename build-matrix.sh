...
# TODO: Use an external matrix for NVIDIA drivers
NVIDIA_VERSIONS=("470.103.01:11-4" "510.47.03:11-6" "515.48.07:11-7")

# Retrieve all the unique kernel versions
KVERS=()
#for y in $(seq 8 11); do
for y in $(seq 11 11); do
    for z in 0-rc.1 $(seq 0 99); do
        for a in "x86_64"; do
...
for KVER_NOARCH in ${KVERS_NOARCH[@]}; do
    ...
    for NV_VER in ${NVIDIA_VERSIONS[@]}; do
        DRIVER=$(echo ${NV_VER} | cut -d ':' -f 1) 
        CUDA=$(echo ${NV_VER} | cut -d ':' -f 2) 

        # Add a comma for all entries but the first one
        [ ${COUNT} -gt 0 ] && echo -n "," >> ${MATRIX_FILE}

        # Add a line for kernel x driver
        echo -n " { \"rhel\": \"${RHEL_VERSION}\", \"ubi-digest\": ${UBI_DIGEST}, \"kernel\": \"${KVER_NOARCH}\", \"driver\": \"${DRIVER}\", \"cuda\": \"${CUDA}\", \"arch\": \"${ARCH}\", \"arch_tag\": \"${ARCH_TAG}\" }" >> ${MATRIX_FILE}
